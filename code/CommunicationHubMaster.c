...
int main(int argc, char ** argv){
    ...
    //Check parameters
    if(argc != 2){
        perr("Invalid number of parameters. Correct usage: ./master.out <number of slaves>");
        exit(1);
    }
    ...
    if(slaves < 0 || slaves > MAX_SLAVES){
        perr("Invalid argument. Max number of slaves is %d.",MAX_SLAVES);
        exit(2);
    }
    ...
    while(1){
        ...
        for(int i = 0; i<r;i++){
            ...
            if(buffer[i]=='<'){
                ...
                //Create command message for the slave
